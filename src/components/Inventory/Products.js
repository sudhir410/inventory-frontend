...
const Products = () => {
  ...
  const handleToggleStatus = async (productId, currentStatus) => {
    try {
      // TODO: Add updateProduct action to Redux slice
              ...
              {products.map((product) => (
                <tr key={product._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="text-sm text-gray-500">Min: {product.stock.minimum}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
      ...
      <div className="bg-white shadow rounded-lg p-4">
        <div className="flex items-center justify-between text-sm text-gray-600">
          ...
          <div className="flex items-center space-x-4">
            <span>{products.filter(p => p.stockStatus === 'Low Stock').length} low stock</span>
            <span>{products.filter(p => p.stockStatus === 'Out of Stock').length} out of stock</span>
          </div>
        </div>
      </div>
    </div>
